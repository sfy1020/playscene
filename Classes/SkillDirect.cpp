...
SkillDirect::SkillDirect():Skill(){}

SkillDirect::~SkillDirect(){
}

SkillDirect::SkillDirect(const SkillDBdata& data, int id, int gx, int gy, int targetID, int damage, int level)
	...
	if(iter == all.end()){
		...
		this->sprite->setScaleY(dis/(float)gtemp.size_y);
